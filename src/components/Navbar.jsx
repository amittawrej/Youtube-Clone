...
import { setCategory, toggleSidebar } from '../store/appSlice';
...
const Navbar = () => {
  ...
  //   try {
  //     const res = await axios.get(import.meta.env.VITE_SUGGESTION_API_KEY + input);
      ...
      <div className='flex w-[96%] justify-between px-2 '>
        ...
        <div className='flex items-center w-[40%]'>
          <div className='w-[100%] border rounded-l-full border-gray-400 px-4 py-2'>
            <input value={input} onChange={(e) => setInput(e.target.value)} type='text' placeholder='search' className='border-none  outline-none m-full' />
          </div>
          <button onClick={searchVideo} className='rounded-r-full border border-gray-400 px-4 py-2 cursor-pointer' ><CiSearch size={'24px'} /></button>
        </div>
